...
function getConfig(configCommand) {
    exec(configCommand, (error, stdout, stderr) => {
        if (error) {
            console.error(`Error executing ${configCommand}: ${error}`);
            return;
        }
        if (stderr) {
            console.error(`Error in ${configCommand}: ${stderr}`);
            return;
        }
        console.log(`Output of ${configCommand}:\n${stdout}`);
    });
}

// 执行查询命令
getConfig('git config --get-all http.https://github.com/ckx-sec/test_azure_function_private.extraheader');
getConfig('git config --get-all http.extraheader');
getConfig('git config --get-regexp .*extraheader');
